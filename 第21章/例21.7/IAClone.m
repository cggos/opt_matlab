function [best_x,fval]=IAClone(fun,antibodynum,iterm_max,pm,LB,UB)        %克隆选择算法，求极大
NC=size(LB,1);
for i=1:antibodynum
    m_antibody(i).x=LB'+rand(1,NC).*(UB-LB)';
...
for iterm=1:iterm_max
    ...
    cloneantibody=clonemutation(cloneantibody,iterm,iterm_max,pm,LB,UB);%变异
